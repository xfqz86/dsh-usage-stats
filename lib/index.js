...
function diffFromDefaults(settings) {
	const patch = {};
	for (const field of Object.keys(USAGE_SETTINGS_DEFAULTS)) if (!settingsValueEqual(settings[field], USAGE_SETTINGS_DEFAULTS[field])) patch[field] = settings[field];
	return patch;
}
/** 把任意数值夹成合法抓取间隔：整数分钟、不低于下限，非法值回退为默认值。 */
function clampGoFetchMinutes(value) {
	return Math.max(3, Math.round(Number.isFinite(value) ? value : 5));
}
/** 把任意数值夹成合法 DeepSeek 抓取间隔：整数分钟、不低于下限，非法值回退为默认值。 */
function clampDeepSeekFetchMinutes(value) {
	return Math.max(3, Math.round(Number.isFinite(value) ? value : 5));
}
/** 把任意数值夹成合法 Z.ai 抓取间隔：整数分钟、不低于下限，非法值回退为默认值。 */
function clampZaiFetchMinutes(value) {
	return Math.max(3, Math.round(Number.isFinite(value) ? value : 5));
}
/**
...
*/
function normalizeModelRedirects(raw) {
...
function normalizeUsageSettings(raw) {
	...
	const minutes = (v, clamp, fallback) => typeof v === "number" && Number.isFinite(v) ? clamp(v) : fallback;
	return {
...
const UsageSettingsSchema = Schema.object({
	...
	goFetchMinutes: Schema.number().default(5).volatile(),
	...
	deepseekFetchMinutes: Schema.number().default(5).volatile(),
	...
	zaiFetchMinutes: Schema.number().default(5).volatile(),
...
*/
function legacySettingsPatch(section) {
	if (section === null || typeof section !== "object" || Array.isArray(section)) return {};
	return diffFromDefaults(normalizeUsageSettings(section));
}
/** 解析旧设置文档文本并取出本插件那一段；文档为空或不是映射时给 undefined。 */
function legacySectionOf(text) {
...
function markLegacySettingsHandled(marker, source, action, fields) {
...
* @param ctx - 服务端插件上下文。
*/
function recoverLegacyUsageSettings(ctx) {
	ctx.inject(["settings"], (settingsCtx) => {
		...
		const run = async () => {
			...
			const userConfigured = entry.user !== void 0 && entry.user !== null && Object.keys(entry.user).length > 0;
...
* openLedger → 先挂 session/event 实时监听 →
...
let UsageStatsService = (() => {
	...
	return class UsageStatsService extends _classSuper {
		...
		*/
		...
		/**
		* 插件条目的 Config：设置服务据此把 `usage-stats` 条目投影成偏好表单，
		* 浏览器端经 ctx.configForms 读同一份取值（见 settings.ts）。
		*/
		static Config = UsageSettingsSchema;
		...
		[Service.init]() {
			...
			recoverLegacyUsageSettings(ctx);
