...
- **清零后重启不再复活历史统计**：此前「清零」只清空插件自己的账本，重启 dsh 后插件初始化发现空库，会全量重扫磁盘上的会话日志，把刚清掉的历史统计原样带回来，与清零提示「不会重新读取历史会话」矛盾。现在清零会在账本里落一个持久标记，重启后不再重扫历史、统计保持归零（清零后的新会话照常实时统计）；想找回历史数据，用「重建」随时可以重新读取历史

## [0.4.1] - 2026-09-14

### 修复

- 会话页「最近活跃」的「今天」误判：此前 24 小时内一律显示「今天 HH:MM」，昨天凌晨的会话到第二天白天看仍显示今天；现在按自然日划分，今天显示「今天」、昨天显示「昨天 HH:MM」、更早显示完整日期
- 「立即刷新」按钮改为真正的强制刷新：此前 3 分钟内点击会直接返回缓存结果、看起来毫无变化，现在点击即重新请求官方接口；自动轮询仍保持原有间隔限制
- OpenCode Go 未订阅时不再误提示「未配置 API Key」，改为如实显示「未开通 OpenCode Go 订阅」，展示与 Z.ai 未开通一致：概览页为空态卡片，未订阅也能手动刷新、会显示更新时间
- 概览页三块额度磁贴（OpenCode Go / Z.ai / DeepSeek）的查询失败提示改为告警样式的卡片；无论正常、未订阅、未配置还是失败，磁贴都常驻「立即刷新」按钮和「更新于」时间，方便随时手动重查并核对接口查询时间
- 侧边栏展开时，额度行在未订阅 / 未配置 Key 的情况下直接显示简短状态（「未订阅」「未配置 Key」），不再只是一个无信息的横杠；折叠时仍显示横杠，悬浮可看完整说明
- 设置页的抓取间隔输入框不再被首屏默认值覆盖：设置数据异步到达后，未编辑过的输入框会跟随服务端取值，避免失焦时把默认值误存回去
- 概览页额度磁贴在数据加载中的一瞬显示「加载中…」，不再误显「未配置 API Key」
- 概览页左列「今日 / 总计」磁贴与热力图上下边缘对齐：卡片纵向填满所在行、内容随高度均布（图标行贴顶、比例条钉底），不再悬浮留白

### 清理

- 全库体检：删除不再使用的代码（旧曲线/分页/文本对齐工具、DeepSeek 今日消耗残留样式等）与未引用的导出；修正多处与实现不符的注释；Z.ai 额度解析去掉不可达分支；扫描与实时入账在元数据写失败时不再连累用量统计；发布文档精简
...
[Unreleased]: https://github.com/xfqz86/dsh-usage-stats/compare/v0.4.0...HEAD
