 ...
 * 本 hook 定期轮询；额度窗口含滚动 5 小时、本周与本月，显示在底部角标
 * 与模态窗详情里。轮询间隔与开关来自偏好设置 settings.ts：
 *   - enabled 为 false 时不发起任何请求，即关闭 Go 额度抓取；
...
export function useGoQuota(
  ...
  // 请求发出，即自身序号不再是最新，丢弃本次结果，防止旧响应覆盖新数据。
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        parsed = await response.json().catch(() => ({})) as { ok?: boolean; value?: GoQuota };
      } catch {
        parsed = {};
      }
  ...
  const refresh = useCallback(() => {
    ...
    void (async () => {
      try {
        ...
        if (parsed.ok === true && parsed.value) {
          setData(parsed.value as GoQuota);
        } else {
          setData((prev) => prev ?? { status: 'error', fetchedAt: Date.now(), rolling: null, weekly: null, monthly: null });
        }
      } catch {
