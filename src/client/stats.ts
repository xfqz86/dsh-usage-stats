...
function isEnglishLocale(localeOrT?: string | ((key: string, params?: Record<string, unknown>) => string)): boolean {
  if (typeof localeOrT === 'function') {
    try {
      ...
      const v = (localeOrT)('panel.summary.callsSuffix');
      if (v === '') return true;
    } catch { /* 忽略 */ }
    try {
      const v2 = (localeOrT)('footer.todayLabel');
...
/** 根据数值大小决定保留小数位：>=100 取0位，>=10 取1位，否则2位。 */
...
export function fmt(n: number | null | undefined, localeOrT?: string | ((key: string, params?: Record<string, unknown>) => string)): string {
  ...
  if (isEn) {
    if (n >= 1e12) { const v = n / 1e12; return trim(v, decimals(v)) + 'T'; }
    if (n >= 1e9) { const v = n / 1e9; return trim(v, decimals(v)) + 'B'; }
    if (n >= 1e6) { const v = n / 1e6; return trim(v, decimals(v)) + 'M'; }
    if (n >= 1e3) { const v = n / 1e3; return trim(v, decimals(v)) + 'K'; }
    return String(Math.round(n));
  }
  if (n >= 1e8) { const v = n / 1e8; return trim(v, decimals(v)) + '亿'; }
  if (n >= 1e4) { const v = n / 1e4; return trim(v, decimals(v)) + '万'; }
  return String(Math.round(n));
}
...
/** 按会话分组：主会话加其子代理，子代理折叠到主会话，孤儿回落顶层，多级展平到根。 */
...
/** 按会话 id 分组：origin 为 subagent 且 parentSession 指向存在会话的子代理折叠到根主会话，孤儿回落顶层，多级展平。 */
export function groupSessions(list: SessionStat[]): SessionGroup[] {
  ...
  const findRoot = (parentId: string): string => {
    ...
    while (true) {
      ...
      if (!node) break;
      const parentSession = node.parentSession;
      if (node.origin !== 'subagent' || typeof parentSession !== 'string' || parentSession === '' || !byId.has(parentSession)) break;
      cur = parentSession;
    }
    return cur;
  };
  ...
  for (const s of list) {
    if (isChild(s)) {
      const parentSession = s.parentSession;
      if (typeof parentSession !== 'string' || parentSession === '') {
        mains.add(s.id);
        continue;
      }
      const rootId = findRoot(parentSession);
      if (byId.has(rootId)) {
...
export interface HeatGridCell {
  ...
  /** 当天调用次数。 */
  calls: number
  /** 强度档 0..4：0 为无用量，底色加边框，1..4 按窗口最大值占比。 */
  lvl: number
  /** 是否今天，外框高亮。 */
  today: boolean
...
/** Codex 风格 26 周热力图：列为周，周一至周日自上而下，列优先。 */
export interface HeatGrid {
  /** 周数，列数。 */
  cols: number
  /** 列优先的格子序列：每周 7 格，周一至周日，从最老的一周排到本周。 */
  cells: HeatGridCell[]
  /** 每列即每周的月份标签：0..11 月索引，与上一列同月时为 null，不显示。 */
  months: (number | null)[]
}

/** 由日序列构建 Codex 风格 26 周热力图，列为周，行为周一至周日。 */
export function heatGridOf(series: SeriesPoint[], weeks = 26): HeatGrid {
  const byDate = new Map<string, SeriesPoint>();
  for (const p of series) if (p?.t != null) byDate.set(fullDayLabel(p.t), p);

  const today = startOfDay(Date.now());
  const end = new Date(today);
  // 对齐到本周周日，getDay() 返回周日为 0至周六为 6，(d+6)%7 映射为周一为 0至周日为 6。
  ...
  for (const c of cells) {
    if (c.v <= 0) continue;
    const ratio = c.v / max;
    if (ratio < 0.25) c.lvl = 1;
    else if (ratio < 0.5) c.lvl = 2;
    else if (ratio < 0.75) c.lvl = 3;
    else c.lvl = 4;
  }
...
export function filterModelsByRange(models: ModelStat[], range: ModelRange): ModelStat[] {
  if (range === 'all') {
  ...
  for (const m of models) {
    ...
    let input = 0; let output = 0; let cacheRead = 0; let cacheWrite = 0; let reasoning = 0; let total = 0; let calls = 0;
...
export interface ModelStackDay {
  ...
  label: string
  total: number
  segments: { provider: string; model: string; total: number; calls: number }[]
}
...
export function buildModelStack(models: ModelStat[], range: ModelRange): ModelStack {
  ...
  const byKey = new Map<string, ModelStat>();
...
/** 日期范围，与模型范围同款枚举，日期 Tab 对齐 ModelsTab 的 7 档，含 1 年。 */
...
export interface DateStackDay {
  ...
  cacheWrite: number
  reasoning: number
  /** 按 token 类型的分段，仅值大于 0 的段，避免渲染空层。 */
  segments: { key: DateTokenKey; label: string; value: number; color: string }[]
}
...
export function buildDateStack(series: SeriesPoint[], range: DateRange, localeT?: (key: string, params?: Record<string, unknown>) => string): DateStack {
  ...
  for (const t of days) {
    ...
    const tokenValues: Record<DateTokenKey, number> = { input, output, cacheRead, cacheWrite, reasoning };
...
/** 饼图整圆判定容差：扇区扫角与 2π 的差小于该值视为覆盖整圆，浮点累计误差加浏览器零长弧省略防护。 */
