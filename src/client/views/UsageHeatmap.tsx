/**
 * 概览 Tab 的 26 周热力图：Codex 风格网格，列为周、行为周一至周日，
 * 含 4 档强度、月份标签与今日高亮。
...
/** 热力图强度 1..4 对应的 scoped 类名，0 为无用量底色。 */
const HEAT_LEVELS = [null, css.hl1, css.hl2, css.hl3, css.hl4] as const;

/** 26 周热力图，位于概览 Tab，每格 tooltip 显示当日明细。 */
export function UsageHeatmap({
  ...
  // 按天索引序列，供 tooltip 取每日明细，缓存为 cacheRead + cacheWrite。
  ...
  /** 单格 tooltip：富插槽排版，日期左对齐、标签左对齐且数值右对齐，顺序为日期、缓存、输入、输出、总计、缓存命中率、调用次数、平均每次调用。 */
