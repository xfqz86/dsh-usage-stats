 ...
 *   @deepseek-ai/dsh-client-ui-sidebar），cell id `dsh-usage-statistics`。
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'dsh-usage-statistics: 文案字典')

  if (ctx.slots === undefined) return

  ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    name: 'sidebar.footer.action',
    id: 'dsh-usage-statistics',
    locale: NS,
    order: 1,
