...
export function Tooltip({
  ...
  side?: TooltipSide
  /** hover 延迟，单位毫秒，键盘聚焦仍立即显示。 */
  delayMs?: number
  /** 为 true 时抑制气泡；锚点仍原样渲染，避免切换时重挂导致过渡中断。 */
  disabled?: boolean
  /** 气泡宽度上限，单位像素，用于长文案超出默认 50vw 的场景。 */
  maxWidth?: number
  /** 为 true 时水平跟随鼠标，即原 FollowTooltip 行为；为 false 时为普通锚点定位。 */
  follow?: boolean
  /** 单个锚点元素；其原有 ref 为回调或对象，会与 Tooltip 的 ref 一并转发。 */
  ...
  function getResolved(): ReactNode {
    ...
    if (typeof raw === 'function') return (raw as () => ReactNode)();
    return raw;
  }
  const resolved: ReactNode = getResolved();

  const isEmpty = resolved === null || resolved === undefined || resolved === '' || (typeof resolved === 'string' && resolved.trim() === '');

  function getY(): number {
    if (pos === null) return 0;
    if (placement === 'right') return pos.top + (pos.bottom - pos.top) / 2;
    if (placement === 'top') return pos.top - 8;
    return pos.bottom + 8;
  }
  const y = getY();
  ...
  // 顶部/底部放不下时翻转到对侧，仅当对侧确实能放下时才翻转，避免震荡。
