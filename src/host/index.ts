 ...
 *   - 首启/重建：扫描会话日志 → 经 foldRecord 写入账本，同步落盘并折
 *     叠聚合缓存，批量物化预统计；
 ...
 * store.ts 聚合折叠、ledger.ts 自管理 sqlite 账本与预统计、scan.ts 日志导入
 * 与重建与预统计物化、snapshot.ts 快照构建、goquota.ts Go 额度、
 * deepseekBalance.ts DeepSeek 余额、zaiQuota.ts Z.ai 额度、http.ts HTTP 辅助与回环围栏。
 *
 * 类型全部来自 harness 包，cordis Context 已合并注入服务表面；运行时
 * 只 import node 内置模块与本地模块。
 */
...
export const inject = {
  ...
  sessionQuery: { required: true },
  sessionPersistence: { required: true },
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        try {
          ...
          if (method === 'go-quota') {
            ...
            const cred = typeof ctx.credentials?.resolve === 'function' ? ctx.credentials : undefined;
            writeOk(res, await queryGoQuota(intervalMinutes, force, cred));
            return;
          }
          if (method === 'deepseek-balance') {
            const raw = payload.intervalMinutes;
            const intervalMinutes = (typeof raw === 'number' && Number.isFinite(raw)) ? raw : undefined;
            // force=true 概览 DeepSeek 磁贴"立即刷新"，绕过 TTL 缓存强制重新抓取；
            // 默认 false 保持轮询语义，有效 TTL 内返回缓存，仍受 3 分钟强制下限保护。
            const force = payload.force === true;
            // 凭据中心为可选注入，inject.credentials.required = false：运行时存在才
            // 走 DSH 凭据中心，否则由查询侧回退 env 与配置文件解析。类型来自
            // harness 合并进 Context 的 CredentialProvider，AGENTS §0。
            const cred = typeof ctx.credentials?.resolve === 'function' ? ctx.credentials : undefined;
            writeOk(res, await queryDeepSeekBalance(intervalMinutes, force, cred));
            return;
          }
          if (method === 'zai-quota') {
            const raw = payload.intervalMinutes;
            ...
            const force = payload.force === true;
            const cred = typeof ctx.credentials?.resolve === 'function' ? ctx.credentials : undefined;
            writeOk(res, await queryZaiQuota(intervalMinutes, force, cred));
            return;
          }
