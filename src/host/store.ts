 ...
 */
import { startOfDay, errorMessage  } from '../utils.ts';
...
export function foldRecord(
  ...
  record: SessionEvent | {
    ...
    data?: { title?: unknown; usage?: unknown };
  },
): void {
  if (record.type === 'session') {
    // 会话种子记录：cwd、createdAt、parentSession 等在顶层，属于 SessionHeader 字段。
  ...
  try {
    ...
    // 畸形事件如缺 message.source，不影响会话其余事件
    return;
  }
  if (ev === null) return;
  ...
  // seq 水位去重：防止初始化扫描与实时监听或重扫双记同一事件。
  if (typeof ev.seq === 'number' && ev.seq >= 0 && ev.seq <= info.maxSeq) {
    store.dedupSkipped += 1;
    return;
  }
  // seq=-1 等未知序事件无法靠水位去重，用主键存在性校验，依 t+session+seq 幂等
  ...
  // 同步更新会话元数据的 lastActive，即事件时间，保证 session_meta 不为空
